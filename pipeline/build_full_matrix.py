...
"""Two models x four cohorts: sensitivity & specificity, overall and per mutation.
  ...
  Model 2  MATRIX-AML multimodal    (single-cell atlas, 8 modalities)  -> scratchpad/oof_metrics.json (all-sc)
...
Writes deliverables/full_matrix.tsv and deliverables/MATRIX-AML_model_x_cohort_matrix.pdf/.png .
...
axA.text(5.4, 1.10, "MATRIX-AML multimodal ②  (single-cell)", ha="center", fontsize=9.5, color=INK, fontweight="bold", transform=axA.transData)
...
fig.suptitle("MATRIX-AML — mutation-calling performance by model and cohort", fontsize=15.5,
...
for ext, dpi in [("pdf", None), ("png", 130)]:
    fig.savefig(os.path.join(OUTD, "MATRIX-AML_model_x_cohort_matrix.%s" % ext),
                **({"dpi": dpi} if dpi else {}), facecolor=SURF)
print("wrote deliverables/MATRIX-AML_model_x_cohort_matrix.pdf/.png + full_matrix.tsv")
print("MATRIX OK")
